...
public class ResourceRepositoryIT {
    ...
    @Test
    public void testSavingAndRetrievingResource() {
        entityManager.getTransaction().begin();
        Resource resource = new Resource("Meeting Room A");
        entityManager.persist(resource);
        entityManager.getTransaction().commit();

        Resource found = entityManager.find(Resource.class, resource.getId());

        assertThat(found.getName()).isEqualTo("Meeting Room A");
    }
}